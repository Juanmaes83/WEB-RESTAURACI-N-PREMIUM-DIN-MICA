/* CLASS 19 · MOTION + MODULE STUDIO INTEGRATION
   ...
   Twelve motion engines exist in this project, and they live behind one product
   library: eight selectable product choreographies, one transversal page motion and
   three complete experiences. The library lets the Studio see and choose them all.

   This is a LIBRARY, not a motion engine. It owns no motion, no geometry and no state:
   ...
   Nothing here is per-engine motion code. The catalogue is data and the renderer
   switches on `kind`. Class 24 is loaded additively here because this library is the
   guaranteed product-level extension point for selectable motion engines; the engine
   itself remains entirely in `class24-half-orbit-selector.js`.
*/
(() => {
  'use strict';
  ...
  /* Class 24 landed after the original runtime chain. Load it additively without
     coupling its geometry or state to this library. */
  if(!document.querySelector('script[data-half-orbit-runtime]')){
    const s=document.createElement('script');s.src='class24-half-orbit-selector.js';
    s.dataset.halfOrbitRuntime='1';document.body.appendChild(s);
  }

  /* ---------- the catalogue ----------
     `kind` is the whole contract:
       preset      → a choreography of the product stage, chosen in the select
  ...
  const ENGINES=[
    ...
    {n:'12',id:'half-orbit',kind:'preset',value:'half-orbit',
      ...
      note:'Media circunferencia tipográfica: drag y flechas hacen un barrido de 180°, cambian el mundo y elevan el producto activo.'}
  ];

  /* Not engines. They are deliberately outside the count of twelve. */
  const MODULES=[
    {id:'location',name:'Location / Google Maps',project:'Class 16 · Class 20',
      note:'Dirección, horarios y mapas con carga bajo consentimiento.'},
    {id:'social-reputation',name:'Social / Reputation',project:'Class 17',
      note:'Prueba social y reputación: reseñas, valoraciones y credibilidad.'},
    {id:'whatsapp-contact',name:'WhatsApp Contact',project:'Class 18',
  ...
  function ensureStyles(){
    ...
    const l=document.createElement('link');l.rel='stylesheet';l.href='styles-v19.css';
    l.dataset.motionLibraryStyles='1';document.head.appendChild(l);
  }
  const label=(badge,state)=>state?`${badge} · ${state}`:badge;

  function card(engine){
    const kind=KINDS[engine.kind],st=statusOf(engine);
    const action=engine.kind==='experience'
      ? `<button type="button" class="ml-action ml-open" data-experience-open="${engine.id}" data-ml-open="${engine.id}">Abrir experiencia →</button>`
      : engine.kind==='page'
        ? `<button type="button" class="ml-action ml-toggle" data-ml-toggle="${engine.id}" aria-pressed="${st.active}">${st.active?'Activado':'Desactivado'}</button>`
        : `<button type="button" class="ml-action ml-activate" data-ml-activate="${engine.id}" ${st.available?'':'disabled'} ${st.active?'aria-current="true"':''}>${st.active?'En uso':'Activar'}</button>`;
    return `<article class="ml-card" data-ml-card="${engine.id}" data-ml-kind="${engine.kind}" data-ml-state="${st.state}">
      <div class="ml-card-top"><span class="ml-n">${engine.n}</span><div class="ml-id"><strong>${engine.name}</strong><span class="ml-project">${engine.project}</span></div></div>
      <p class="ml-note">${engine.note}</p>
      <div class="ml-card-foot"><span class="ml-state" data-ml-state-label>${label(kind.badge,st.state)}</span>${action}</div>
    </article>`;
  }

  function moduleCard(mod){
    return `<article class="ml-card ml-card-module" data-ml-card="${mod.id}">
      <div class="ml-card-top"><span class="ml-n ml-n-module">M</span><div class="ml-id"><strong>${mod.name}</strong><span class="ml-project">${mod.project}</span></div></div>
      <p class="ml-note">${mod.note}</p>
      <div class="ml-card-foot"><span class="ml-state" data-ml-module-state="${mod.id}">MÓDULO · OFF</span><button class="ml-action" type="button" data-configure-module="${mod.id}">Configurar</button></div>
    </article>`;
  }
  ...
  function build(){
    ...
    ensureStyles();
    library=document.createElement('section');library.className='ml-library';
    library.innerHTML=`
      <header class="ml-head"><div><p class="eyebrow">Motion Engines</p><h3>Biblioteca de motores <span class="ml-count">${ENGINES.length}</span></h3><p class="ml-lead">Todos los lenguajes de movimiento del proyecto en un solo sitio. Las coreografías se activan sobre la carta; el movimiento de página convive con cualquiera de ellas; las experiencias completas se abren en su propia pantalla.</p></div></header>
      <div class="ml-filters" role="tablist" aria-label="Filtrar motores">${FILTERS.map(f=>`<button type="button" role="tab" class="ml-filter" data-ml-filter="${f.id}" aria-selected="${f.id===filter}">${f.label}</button>`).join('')}</div>
      <div class="ml-grid" data-ml-grid>${ENGINES.map(card).join('')}</div>
      <section class="ml-modules"><h4>Módulos <span class="ml-count ml-count-soft">${MODULES.length}</span></h4><p class="ml-lead">No son motores de movimiento y no cuentan dentro de los ${ENGINES.length}: son piezas de negocio que se integran aparte.</p><div class="ml-grid">${MODULES.map(moduleCard).join('')}</div></section>`;
    ...
    library.addEventListener('click',e=>{const b=e.target.closest('[data-experience-open]');if(b)window.RestaurantExperienceShell?.open(b.dataset.experienceOpen,{trigger:b})});
    sync();root.dataset.motionLibrary='ready';return true;
  }

  function wire(){
    library.addEventListener('click',e=>{
      const f=e.target.closest('[data-ml-filter]');
      if(f){filter=f.dataset.mlFilter;$$('[data-ml-filter]',library).forEach(b=>b.setAttribute('aria-selected',String(b.dataset.mlFilter===filter)));applyFilter();return}
      const act=e.target.closest('[data-ml-activate]');
      if(act&&!act.disabled){const engine=ENGINES.find(x=>x.id===act.dataset.mlActivate);activate(engine);return}
      const tog=e.target.closest('[data-ml-toggle]');
      if(tog){const engine=ENGINES.find(x=>x.id===tog.dataset.mlToggle);const now=window.RestaurantStudioConfig?.get?.(engine.path)!==false;window.RestaurantStudioConfig?.set?.(engine.path,!now)}
    });
  ...
  function activate(engine){
    const s=select();if(!s||!engine||!optionFor(engine.value))return false;
    s.value=engine.value;s.dispatchEvent(new Event('input',{bubbles:true}));s.dispatchEvent(new Event('change',{bubbles:true}));
    window.RestaurantMotionStudio?.publish?.();sync();return true;
  }
  function applyFilter(){
    $$('[data-ml-card]',library).forEach(c=>{if(!c.classList.contains('ml-card-module'))c.hidden=filter!=='all'&&c.dataset.mlKind!==filter});
  }
  function sync(){
    if(!library)return;
    ...
    ENGINES.forEach(engine=>{
      ...
      const btn=$('.ml-activate',el);if(btn){btn.disabled=!st.available;btn.textContent=st.active?'En uso':'Activar';if(st.active)btn.setAttribute('aria-current','true');else btn.removeAttribute('aria-current')}
      const toggle=$('.ml-toggle',el);if(toggle){toggle.setAttribute('aria-pressed',String(st.active));toggle.textContent=st.active?'Activado':'Desactivado'}
    });
    const active=ENGINES.find(e=>e.kind==='preset'&&statusOf(e).active);root.dataset.motionLibraryActive=active?active.id:'';applyFilter();
  }

  function boot(){
    if(ready)return;const panel=$('.studio-panel.motion-panel');if(!panel||panel.offsetParent===null)return;
  ...
  window.RestaurantMotionLibrary={
    engines:()=>ENGINES.map(e=>({...e,status:statusOf(e)})),modules:()=>MODULES.slice(),count:()=>ENGINES.length,moduleCount:()=>MODULES.length,
    activate(id){const e=ENGINES.find(x=>x.id===id);return e?activate(e):false},
    setFilter(id){filter=id;if(library){$$('[data-ml-filter]',library).forEach(b=>b.setAttribute('aria-selected',String(b.dataset.mlFilter===filter)));applyFilter()}return filter},
    state(){return {ready,count:ENGINES.length,modules:MODULES.length,filter,active:root.dataset.motionLibraryActive||'',available:ENGINES.filter(e=>statusOf(e).available).length}}
  };
})();