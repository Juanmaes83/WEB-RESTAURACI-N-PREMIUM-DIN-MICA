...
(() => {
  ...
  function field(label,path,{type='text',options=null,value=''}={}){
    ...
    input.dataset.bevPath=path;if(type==='checkbox')input.checked=!!value;else input.value=value??'';
    const event=(type==='checkbox'||options)?'change':'input';input.addEventListener(event,()=>{let v=type==='checkbox'?input.checked:input.value;if(type==='number')v=input.value===''?null:Number(input.value);set(path,v)});
  ...
  function removeItem(id){set(`${PATH}.items`,items().filter(x=>x.id!==id));if(openId===id)openId=null;render()}
  function move(id,delta){const next=items(),from=next.findIndex(x=>x.id===id),to=from+delta;if(from<0||to<0||to>=next.length)return;[next[from],next[to]]=[next[to],next[from]];set(`${PATH}.items`,next);openId=id;render()}
  async function upload(id,file){if(!file||!file.type.startsWith('image/')){status('Selecciona una imagen.');return}const mediaId=B().newMediaId('image'),ref=B().refFor(id,mediaId);await media().save(ref,file);updateItem(id,item=>{item.media=[{id:mediaId,kind:'image',ref,alt:item.name||''},...(item.media||[]).filter(m=>m.kind!=='image')]});status('Imagen vinculada desde la Media Library.')}
  ...
  function card(item,index,total){
    ...
    specs.forEach(([label,key,type])=>{let input;if(type==='select')input=field(label,'',{options:B().TYPES.map(v=>[v,v]),value:item[key]});else if(type==='motion')input=field(label,'',{options:B().MOTION.map(v=>[v,v]),value:item[key]});else input=field(label,'',{type,value:key.includes('.')?key.split('.').reduce((a,k)=>a?.[k],item):item[key]});const control=input.querySelector('input,textarea,select');control.removeAttribute('data-bev-path');const write=()=>updateItem(item.id,x=>{const parts=key.split('.');const last=parts.pop();const target=parts.reduce((a,k)=>(a[k]??={}),x);target[last]=control.type==='checkbox'?control.checked:control.value});control.oninput=write;control.onchange=write;grid.append(input)});
    d.append(grid);
  ...
  function render(){if(!built)return;const s=state();panel.querySelector('[data-bev-list]').innerHTML='';const list=panel.querySelector('[data-bev-list]');s.items.forEach((item,i)=>list.append(card(item,i,s.items.length)));panel.querySelectorAll('[data-bev-global]').forEach(input=>{if(document.activeElement===input)return;const v=get(input.dataset.bevGlobal);if(input.type==='checkbox')input.checked=!!v;else input.value=v??''})}
  function build(){
    ...
    g.forEach(([label,key,type])=>{const options=type==='preset'?B().PRESETS.map(v=>[v,v]):null;const f=field(label,`${PATH}.${key}`,{type:type==='preset'?'text':type,options,value:state()[key]});const input=f.querySelector('input,textarea,select');input.dataset.bevGlobal=`${PATH}.${key}`;g.append(f)});panel.append(g);
