...
(() => {
  ...
  const DEFAULTS = Object.freeze({
    enabled:false,
    preset:'dynamic-selector',
    eyebrow:'Bebidas',
    title:'La carta también se bebe.',
    intro:'',
    ctaLabel:'Ver carta',
    ctaUrl:'',
    showPrice:true,
  ...
  const item = (patch={}) => ({
    ...
    motionPreset:MOTION.includes(patch.motionPreset) ? patch.motionPreset : 'neutral',
    theme:{...clone(DEFAULT_THEME),...(patch.theme || {})},
  ...
  function normalize(value){
    ...
    return {
      ...
      ctaLabel:typeof v.ctaLabel==='string' ? v.ctaLabel : DEFAULTS.ctaLabel,
      ctaUrl:typeof v.ctaUrl==='string' ? v.ctaUrl : '',
      showPrice:v.showPrice !== false,
      showPairing:v.showPairing !== false,
      ambientParticles:v.ambientParticles !== false,
  ...
  function seed(){
    if(window.RestaurantDefaults && !window.RestaurantDefaults.beverages)
      window.RestaurantDefaults.beverages=clone(DEFAULTS);
    const cfg=window.RestaurantStudioConfig;
    if(!cfg?.get) return false;
    const live=cfg.get('beverages');
    if(!live){
      /* late-loaded branch review: schema migration only; no user edit, no parallel persistence */
      const root=cfg.get('');
      if(root && typeof root==='object'){ root.beverages=clone(DEFAULTS); return true; }
      return false;
    }
    const normalized=normalize(live);
    Object.keys(normalized).forEach(k=>{ if(live[k]===undefined) live[k]=clone(normalized[k]); });
    return false;
  }
  if(window.RestaurantDefaults && !window.RestaurantDefaults.beverages)
    window.RestaurantDefaults.beverages=clone(DEFAULTS);
