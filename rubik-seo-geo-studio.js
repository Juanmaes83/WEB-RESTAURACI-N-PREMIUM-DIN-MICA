...
(() => {
  ...
  function render(){
    ...
    const mediaList=panel.querySelector('[data-seo-media-list]');if(mediaList&&window.RubikSEOGeoMedia){mediaList.replaceChildren();const state=api().snapshot(),refs=new Set(Object.keys(state.media||{}));(state.dishes||[]).forEach(d=>{if(d.image)refs.add(d.id)});for(const ref of refs){const a=RubikSEOGeoMedia.project(state,ref),row=el('article','seo-media-row');row.append(el('strong','',a.originalName));for(const key of ['seoFilename','alt','caption','title']){const input=el('input','');input.value=a[key].value;input.dataset.mediaRef=ref;input.dataset.mediaKey=key;input.addEventListener('change',()=>api().set(`seo.media.${ref}.${key}`,{...api().get(`seo.media.${ref}.${key}`),mode:'custom',value:input.value,updatedAt:new Date().toISOString()}));row.append(input);}const toggle=el('button','studio-primary',a.decorative?'Informativa':'Marcar decorativa');toggle.type='button';toggle.onclick=()=>api().set(`seo.media.${ref}.decorative`,!a.decorative);row.append(toggle,el('small','seo-help',`${a.seoFilename.mode.toUpperCase()} · ${a.alt.decisionReason} · relaciones: ${(a.subjectEntityRefs||[]).join(', ')||'pendientes'}`));mediaList.append(row);}}
  }
