...
try{
  ...
  await page.locator('.studio-nav [data-panel="motion"]').click();await page.waitForFunction(()=>window.RestaurantMotionLibrary?.state().ready&&RestaurantMotionLibrary.state().available===12);check('Class 19 exposes twelve approved engines including Half Orbit',await page.evaluate(()=>RestaurantMotionLibrary.state().count===12&&!!document.querySelector('[data-ml-card="half-orbit"]')));
  // Class24 made Traveler opt-in. Explicitly enable the coexistence fixture.
  await page.evaluate(()=>RestaurantStudioConfig.set('scrollTraveler.enabled',true));await waitSave();
