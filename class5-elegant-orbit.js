...
(() => {
  ...
  function bind(){const next=document.querySelector('#next-dish'),prev=document.querySelector('#prev-dish');next?.addEventListener('click',()=>{if(isElegant())dance(1,'button')},true);prev?.addEventListener('click',()=>{if(isElegant())dance(-1,'button')},true);shell.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)>=Math.abs(e.deltaX))return;if(isElegant())dance(e.deltaX>=0?1:-1,'wheel')},{passive:true,capture:true});shell.addEventListener('keydown',e=>{if(!isElegant()||detailIsOpen())return;if(e.key==='ArrowRight')dance(1,'keyboard');if(e.key==='ArrowLeft')dance(-1,'keyboard')},true);shell.addEventListener('restaurant:orbit-drag-start',e=>{if(!isElegant()||detailIsOpen())return;dragStartX=e.detail.startX;stopBreathing()});shell.addEventListener('pointercancel',()=>{if(!isElegant()||dragStartX===null)return;dragStartX=null;startBreathing()});shell.addEventListener('pointerup',e=>{if(!isElegant()||detailIsOpen()||dragStartX===null)return;const dx=e.clientX-dragStartX;dragStartX=null;if(Math.abs(dx)>16)dance(dx<0?1:-1,'drag');else startBreathing()},{passive:true,capture:true});stage.addEventListener('click',e=>{if(!isElegant()||detailIsOpen())return;const dish=e.target.closest('.orbit-dish');if(!dish)return;const c=shellCenter(),r=dish.getBoundingClientRect(),dx=r.left+r.width/2-c.x;if(Math.abs(dx)>20)dance(dx>0?1:-1,'dish')},true)}
