...
const runtimes=['class8-depth-carousel.js','class9-anchor-scenes.js','class10-orbital-food.js',
  ...
  'class24-half-orbit-selector.js','native-product-engines.js'];
...
/* ---- 2. the modules are listed and are NOT Motion elements ----
...
for(const id of modules){
  if(entries.some(e=>e.id===id))fail(`${id} is counted both as a module and as a Motion element`);
  const key=KEY[id]||id;
...
if(!guard.includes("s.src='native-product-engines.js'")&&!guard.includes("s.src='class19-motion-library.js'"))
  fail('the native Product Engines have no additive runtime path');
...
for(const f of ['class8-depth-carousel.js','class10-orbital-food.js','class11-pizza-slice-orbit.js',
  'class14-scroll-traveler.js','class24-half-orbit-selector.js','native-product-engines.js']){
